...
def completed_boundary(server_now: datetime) -> datetime:
    ...
    return datetime.fromtimestamp(timestamp - timestamp % 300, UTC)


def next_poll_at(server_now: datetime, delay: timedelta = POLL_DELAY) -> datetime:
    ...
    return completed_boundary(server_now) + BAR_LENGTH + delay


def build_latest_bar_request(contract: QualifiedContract, server_now: datetime) -> HistoricalRequest:
    """Build a 300-second request ending at the latest completed boundary."""
    boundary = completed_boundary(server_now)
    target_start = boundary - BAR_LENGTH
...
class LatestBarPoller:
    ...
    def poll_once(self) -> HistoricalBar:
        ...
        request = build_latest_bar_request(self.contract, server_now)
        collected = self.collector.collect(request, self.timeout_seconds)
        normalized = tuple(
            normalize_completed_bar(raw, self.contract, server_now)
